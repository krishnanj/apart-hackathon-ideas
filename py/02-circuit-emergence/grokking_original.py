#!/usr/bin/env python3
"""
Original Grokking Setup: Replicate the exact conditions from the paper
"""
...
def run_original_grokking():
    """Run grokking experiment with original paper setup"""
    
    print("Starting original grokking experiment...")
    print("Using exact setup from 'Grokking: Generalization beyond overfitting'")
    ...
    train_x, train_y, test_x, test_y = create_original_grokking_dataset(p, subset_ratio)
    ...
    for step in range(n_steps):
        ...
        if step % 1000 == 0:
            ...
            train_acc = compute_accuracy(model(train_x.float()), train_y.long())
            test_acc = compute_accuracy(model(test_x.float()), test_y.long())
    ...
    circuit_analysis = analyze_circuit_development_original(grokking_checkpoints, test_x, test_y)
    
    # Create grokking plots
    create_original_grokking_plots(train_losses, test_losses, train_accuracies, test_accuracies, 
                                  circuit_analysis, checkpoint_interval)
    
    print("Original grokking experiment completed!")

def create_original_grokking_dataset(p, subset_ratio):
    """Create dataset with original paper setup"""
...
def analyze_circuit_development_original(checkpoints, test_x, test_y):
    """Analyze how internal circuits develop during grokking"""
    circuit_results = {}
    
    # Define concepts to probe for (same as before)
    concepts = {
...
def create_original_grokking_plots(train_losses, test_losses, train_accuracies, test_accuracies, 
                                 circuit_analysis, checkpoint_interval):
    ...
    plt.ylabel('Loss')
    plt.title('Loss During Training (Original Setup)')
    plt.legend()
    ...
    plt.xlabel('Training Steps')
    plt.ylabel('Accuracy')
    plt.title('Accuracy During Training (Original Setup)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title('Grokking Detection (Original Setup)')
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plot_filename = f"plots/original_grokking_analysis_{timestamp}.png"
    ...
    results_filename = f"results/original_grokking_results_{timestamp}.pt"
    save_dict_as_pt(results, results_filename)
    print(f"Saved: {results_filename}")

if __name__ == "__main__":
    run_original_grokking() 